...
def process_post(
    ...
    # Resolve source id: use the single global '4chan' source only
    src_name_used = "4chan"
    ...
    # Now perform a single LLM call per asset after all validation passed
...
def main():
    ...
    # Track thread last_modified per board to limit requests, and seen post ids to dedupe
    last_modified_by_thread: Dict[str, Dict[int, int]] = {b: {} for b in boards}
    seen_post_ids: Set[int] = set()

    info("fourchan_stream_start", boards=boards)
    while True:
        try:
            ...
            for board in boards:
                ...
                for page in pages:
                    ...
                    for t in threads:
                        ...
                        # Compute cutoff for skipping old posts
                        ...
                        for p in posts:
                            ...
                            if pt and pt < cutoff_ts:
                                skipped_old += 1
                                continue
                            # Ensure DB connection per batch to stay fresh
                            conn = ensure_db_connection(conn, s.DATABASE_URL)
                            try:
            ...
            err("fourchan_loop_error", error=str(e))
